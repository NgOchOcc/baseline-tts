"""
Baseline evaluation testing ALL output indices (0-31) using SMART verification.
For each output index, calculate accuracy separately using output[idx] as final answer.
Uses verify_answer_smart() for exact numeric match + format normalization.
"""
...
def evaluate_all_indices_smart():
    """Evaluate accuracy for each output index (0-31) using smart verification."""
    ...
    print("=" * 100)
    print("BASELINE EVALUATION (SMART): Testing each output index (0-31)")
    print("=" * 100)
    print()
    ...
    for q_idx, question_dir in enumerate(question_dirs):
        ...
        for output_idx in range(min(32, len(outputs))):  # Cap at 32 indices
            try:
                ...
                # Verify answer using smart verification (exact numeric + format normalization)
                if baseline_answer:
    ...
    # Print summary table
    print("\n" + "=" * 100)
    print("RESULTS (SMART): Accuracy for each output index (0-31)")
    print("=" * 100)
    print()
    ...
    output_file = base_dir / "baseline_result_all_indices_smart.json"
